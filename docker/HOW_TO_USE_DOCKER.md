...
Use the Dockerized version of AdevHub-AI for a much faster and complete startup of AdevHub-AI.

### Minimum Requirements

> [!TIP]
> Running AdevHub-AI on AWS/GCP/Azure?
...
\*AdevHub-AI by default uses a built-in vector database powered by [LanceDB](https://github.com/lancedb/lancedb)

\*AdevHub-AI by default embeds text on instance privately [Learn More](../server/storage/models/README.md)

## Recommend way to run dockerized AdevHub-AI!
...
> the docker container using AdevHub-AI as `localhost:xxxx` will not resolve for the host system.
>
...
```shell
docker pull mintplexlabs/AdevHub-AI
```

<table>
<tr>
<th colspan="2">Mount the storage locally and run AdevHub-AI in Docker</th>
</tr>
<tr>
<td>
  Linux/MacOs
...
```shell
export STORAGE_LOCATION=$HOME/AdevHub-AI && \
...
$env:STORAGE_LOCATION="$HOME\Documents\AdevHub-AI"; `
...
services:
  AdevHub-AI:
    image: mintplexlabs/AdevHub-AI
    container_name: AdevHub-AI
    ports:
    - "3001:3001"
    cap_add:
      - SYS_ADMIN
    ...
    volumes:
      - AdevHub-AI_storage:/app/server/storage
    restart: always
...
Go to `http://localhost:3001` and you are now using AdevHub-AI! All your data and progress will persist between
...
- `touch server/storage/AdevHub-AI.db` to create empty SQLite DB file.
- `cd docker/`
...
The integrations below are templates or tooling built by the community to make running the docker experience of AdevHub-AI easier.

### Use the Midori AI Subsystem to Manage AdevHub-AI

Follow the setup found on [Midori AI Subsystem Site](https://io.midori-ai.xyz/subsystem/manager/) for your host OS
After setting that up install the AdevHub-AI docker backend to the Midori AI Subsystem.
...
Then in docker you need to replace that localhost part with `host.docker.internal`. For example, if running Ollama on the host machine, bound to http://127.0.0.1:11434 you should put `http://host.docker.internal:11434` into the connection URL in AdevHub-AI.
