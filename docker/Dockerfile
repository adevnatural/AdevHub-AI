...
RUN echo "Preparing build of AdevHub-AI image for arm64 architecture"
...
RUN (getent passwd "$ARG_UID" && userdel -f "$(getent passwd "$ARG_UID" | cut -d: -f1)") || true && \
    ...
    groupadd -g "$ARG_GID" AdevHub-AI && \
    useradd -l -u "$ARG_UID" -m -d /app -s /bin/bash -g AdevHub-AI AdevHub-AI && \
    mkdir -p /app/frontend/ /app/server/ /app/collector/ && chown -R AdevHub-AI:AdevHub-AI /app

# Copy docker helper scripts
COPY ./docker/docker-entrypoint.sh /usr/local/bin/
COPY ./docker/docker-healthcheck.sh /usr/local/bin/
COPY --chown=AdevHub-AI:AdevHub-AI ./docker/.env.example /app/server/.env
...
RUN echo "Need to patch Puppeteer x Chromium support for ARM86 - installing dep!" && \
    curl -fSL https://webassets.AdevHub-AI.com/chromium-1088-linux-arm64.zip -o chrome-linux.zip && \
...
RUN echo "Preparing build of AdevHub-AI image for non-ARM architecture"
...
RUN echo "Running common build flow of AdevHub-AI image for all architectures"
...
COPY --chown=AdevHub-AI:AdevHub-AI ./server /app/server/
WORKDIR /app/server
RUN yarn install --production --network-timeout 100000 && yarn cache clean
WORKDIR /app

# Install collector dependencies
COPY --chown=AdevHub-AI:AdevHub-AI ./collector/ ./collector/
...
WORKDIR /app
COPY --chown=AdevHub-AI:AdevHub-AI --from=frontend-build /app/frontend/dist /app/server/public
